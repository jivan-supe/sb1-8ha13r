import { Globe, Award, Zap, Check, ListChecks } from 'lucide-react';
import { Competitor } from '../../types/competitor';
import SimilarityBadge from './SimilarityBadge';
import FeatureList from './FeatureList';

interface SelectableCompetitorCardProps {
  competitor: Competitor;
  isSelected: boolean;
  onToggleSelect: (id: string) => void;
}

export default function SelectableCompetitorCard({ 
  competitor, 
  isSelected,
  onToggleSelect 
}: SelectableCompetitorCardProps) {
  return (
    <div 
      className={`bg-white rounded-lg shadow-md p-6 hover:shadow-lg transition-all duration-200 cursor-pointer relative
        ${isSelected ? 'ring-2 ring-indigo-600' : ''}`}
      onClick={() => onToggleSelect(competitor.id)}
    >
      {isSelected && (
